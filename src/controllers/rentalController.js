...
export async function postRental(req, res){
    ...
    const rentalSchema = joi.object({
     ...
     const { error } = rentalSchema.validate(newRental);
      if (error) {
        ...
        const{ rows: priceGame } = await connection.query(`SELECT "pricePerDay" FROM games WHERE id=${newRental.gameId}`)
        const price = priceGame[0].pricePerDay
        const priceRental = (parseInt(price) * parseInt(newRental.daysRented))

        const{ rows: stockTotal } = await connection.query(`SELECT "stockTotal" FROM games WHERE id=${newRental.gameId}`)
    ...
    try {
        ...
        await connection.query(`UPDATE games SET "stockTotal"='${stock-1}' WHERE id = ${newRental.gameId}`)
...
export async function deleteRental(req, res){
    ...
    try {
        ...
        const {rows:returned} = await connection.query(`SELECT "returnDate" FROM rentals WHERE id=${id} `)
...
export async function finishRentals(req, res){
    ...
    const {rows:gameId} = await connection.query(`SELECT "gameId" FROM rentals WHERE id=${id} `)
    const gameIdValue = gameId[0].gameId
    const{ rows: stockTotal } = await connection.query(`SELECT "stockTotal" FROM games WHERE id=${gameIdValue}`)
    const stock = stockTotal[0].stockTotal
   
    const convertmilisindays = 86400000
    const {rows: dayreturn} = await connection.query(`SELECT "daysRented" FROM rentals WHERE id=${id}`)
    ...
    const {rows: price} = await connection.query(`SELECT "originalPrice" FROM rentals WHERE id=${id}`)
    const priceday = price[0].originalPrice
    const fee = delayDays * priceday

    if(delayDays > 0){
        await connection.query(`UPDATE rentals SET "delayFee"='${fee}' WHERE id = ${id}`)
     
    }
    
        try {
            await connection.query(`UPDATE rentals SET "returnDate"='${today}' WHERE id = ${id}`)
            await connection.query(`UPDATE games SET "stockTotal"='${stock+1}' WHERE id = ${gameIdValue}`)
            res.sendStatus(201)
        } catch (error) {
            console.log(error)
            res.status(500).send("Erro no servidor") 
